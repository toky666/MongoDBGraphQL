...
function isResolvedPreset(preset) {
    ...
    if (!preset.plugins)
        return false;
    if (!preset.disablePlugins)
        return false;
    if (typeof preset.lib !== "object" || preset.lib === null)
        return false;
    const seenPlugins = new Map();
    for (const plugin of preset.plugins) {
        if (preset.disablePlugins.includes(plugin.name))
            return false;
...
function resolvePreset(preset) {
    ...
    const disabledButNotSeen = resolvedPreset.disablePlugins?.filter((n) => !seenPlugins.has(n));
...
function resolvePresetsInternal(presets, seenPlugins, depth) {
    ...
    for (const preset of presets) {
        ...
        const resolvedPreset = resolvePresetInternal(preset, seenPlugins, depth + 1);
 ...
 * @internal
 */
function resolvePresetInternal(preset, seenPlugins, depth) {
